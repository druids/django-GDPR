...
class TestDeactivateExpiredReasons(AnonymizedDataMixin, TestCase):
    ...
    def test_command_should_expire_active_legal_reson(self):
        ...
        self.assertTrue(LegalReason.objects.exists_valid_consent(MARKETING_SLUG, self.customer))

        # consent has expired
        with freeze_time(legal_reason.expires_at + relativedelta(seconds=1)):
            test_call_command('deactivate_expired_reasons')
        self.assertFalse(LegalReason.objects.exists_valid_consent(MARKETING_SLUG, self.customer))
    ...
    def test_command_should_not_expire_deactivated_legal_reasons(self):
        ...
        # consent has not expired
        test_call_command('deactivate_expired_reasons')
        self.assertTrue(LegalReason.objects.exists_deactivated_consent(MARKETING_SLUG, self.customer))
    ...
    def test_command_should_not_anonymize_customer_if_expiring_reason_is_not_related_to_customer_date(self):
        ...
        test_call_command('deactivate_expired_reasons')
        self.assertTrue(LegalReason.objects.exists_valid_consent(MARKETING_SLUG, self.customer))
        self.assertTrue(LegalReason.objects.exists_valid_consent(FIRST_AND_LAST_NAME_SLUG, self.customer))
        ...
        self.assertFalse(LegalReason.objects.exists_valid_consent(MARKETING_SLUG, self.customer))
        self.assertTrue(LegalReason.objects.exists_valid_consent(FIRST_AND_LAST_NAME_SLUG, self.customer))
        self.assertEqual(self.customer.first_name, CUSTOMER__FIRST_NAME)
        self.assertAnonymizedDataNotExists(self.customer, 'first_name')
        self.assertNotEqual(self.customer._anonymize_obj(), CUSTOMER__FIRST_NAME)
